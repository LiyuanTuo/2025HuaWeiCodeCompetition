...
int N, g[11], k[11], m[11], M, s[501], e[501], cnt[501], latency[11][501], a, b, server_index[501][11], NPU_size[11][11][200000], request_size[11];
...
void solution()
{
    for (int i = 1; i <= M; i++)
    {
        int Ti = 0, serverj = server_index[i][1];
        // Bj = request_size[serverj]
        Ti = (int)ceil(1.0 * cnt[i] / request_size[serverj]);
        int timej = s[i];
        ...
        for (int k = 1; k <= Ti; k++)
        {
            cout << timej << " " << serverj << " " << which_gpu[serverj] << " " << min(request_size[serverj], cnt[i]) << " ";

            timej += latency[serverj][i] + 1;
            cnt[i] -= min(request_size[serverj], cnt[i]);
